...
export const createTransaction = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const seatAvailability = await SeatModel.find({ _id: { $in: seats }, status: 'available' });
        ...
        await SeatModel.updateMany({ _id: { $in: seats } }, { status: 'inTransaction' });
...
export const getUserTransactions = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const transactions = await TransactionModel.find({ userId: user._id }).populate('seats').populate('userId');
...
export const getTransactionById = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const { id } = req.params;
        const transaction = await TransactionModel.findById(id).populate('seats').populate('userId');
...
export const updateTransactionStatus = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        if (status === 'cancelled') {
            await SeatModel.updateMany({ _id: { $in: transaction.seats } }, { status: 'available' });
        }
